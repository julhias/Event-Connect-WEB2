...
function BottomNavigation() {
  ...
  return (
    ...
    <nav className="lg:hidden bg-white shadow-[0_-2px_10px_rgba(0,0,0,0.05)] flex justify-around py-3 border-t border-gray-100 fixed bottom-0 w-full z-50 left-0">
      ...
      <Link 
        to="/eventos" 
        className={`flex flex-col items-center justify-center w-1/4 transition-colors duration-200 ${isActive('/eventos') ? activeClass : inactiveClass}`}
      >
        <svg className="h-6 w-6 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <Link 
        to="/prestadores" 
        className={`flex flex-col items-center justify-center w-1/4 transition-colors duration-200 ${isActive('/prestadores') ? activeClass : inactiveClass}`}
      >
         <svg className="h-6 w-6 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
